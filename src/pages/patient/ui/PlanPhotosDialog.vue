...
// Получаем фото плана лечения
const photos = computed(() => {
  if (!props.patient || !props.plan) return []
  return patientStore.getPhotosByPlanId(props.patient.id, props.plan.id)
})
...
watch(() => props.modelValue, async (isOpen) => {
  if (isOpen && props.patient && props.plan) {
...
async function uploadPhotos() {
  if (!props.patient || !props.plan || selectedFiles.value.length === 0) {
    return
  }
  ...
  try {
    ...
    for (const file of selectedFiles.value) {
      try {
        await patientStore.uploadPhoto(props.patient!.id, file, undefined, props.plan!.id)
        successCount++
    ...
    <v-card class="photos-dialog-card">
      ...
      <v-card-text class="pa-0" style="height: calc(100vh - 120px); overflow: hidden;">
        ...
        <div v-if="photos.length === 0 && photoPreviews.length === 0" class="empty-photos-state">
          ...
          <p class="text-body-1 text-grey mb-4">Фото "до" лечения еще не добавлены</p>
          <v-btn
            color="primary"
            prepend-icon="mdi-camera"
            @click="fileInput?.click()"
